...
#include <cstdint>
enum class OPPONENT : int_fast8_t
{
    AI,
    HUMAN
};
...
namespace clicked
{
const sf::Texture blackClicked(sf::Image({BLACK_CLICKED_WIDTH, BLACK_CLICKED_HEIGHT}, Black_Clicked));
const sf::Texture WhiteClicked(sf::Image({WHITE_CLICKED_WIDTH, WHITE_CLICKED_WIDTH}, White_Clicked));
...
namespace normal
{
const sf::Texture blackNormal(sf::Image({BLACK_NORMAL_WIDTH, BLACK_NORMAL_HEIGHT}, Black_Normal));
const sf::Texture whiteNormal(sf::Image({WHITE_NORMAL_WIDTH, WHITE_NORMAL_HEIGHT}, White_Normal));
static const inline sf::Sprite white()
{
...
{
    sf::RenderWindow window(sf::VideoMode({WINDOW_WIDTH, WINDOW_HEIGHT}), "Chess", sf::Style::Close);
    const sf::VideoMode desktopMode{sf::VideoMode::getDesktopMode()};
    const int centerX{static_cast<int>(desktopMode.size.x / 2 - WINDOW_WIDTH / 2)};
    const int centerY{static_cast<int>(desktopMode.size.y / 2 - WINDOW_HEIGHT / 2)};
...
namespace board
{
const sf::Texture blackBoardTexture(sf::Image({CHESS_BLACK_PERSPECTIVE_BOARD_WIDTH,
                                               CHESS_BLACK_PERSPECTIVE_BOARD_HEIGHT},
                                              Chess_black_perspective_board));
const sf::Texture whiteBoardTexture(sf::Image({CHESS_WHITE_PERSPECTIVE_BOARD_WIDTH,
                                               CHESS_WHITE_PERSPECTIVE_BOARD_HEIGHT},
                                              Chess_white_perspective_board));
static const inline sf::Sprite black()
{
    const sf::Vector2f boardPosition{static_cast<float>(WINDOW_WIDTH / 2.0 - blackBoardTexture.getSize().x / 2.0),
                                     static_cast<float>(WINDOW_HEIGHT / 2.0 - blackBoardTexture.getSize().y / 2.0)};
